...
BEGIN
	...
	DECLARE pres_cur cursor for
		select pres_id, (pres_max_students - pres_enrolled_students) as remaining
		from presentation
		where pres_enrolled_students < pres_max_students
		and ses_id = sesId
		and pres_id not in (
            select pres_id
...
#        set result = @p1;
	...
	student_loop: LOOP
		...
		insert into user_presentation_xref (pres_id, usr_id, presenting, user_pres_updt_usr_id)
        values (presId, userId, 0, -1);
        commit;
